...
const RANDOM_NAME_CANONICAL = `https://usonlinetools.com${getCanonicalToolPath("random-name-generator")}`;

const RANDOM_NAME_FAQS = [
  {
  ...
  {
    q: "Can I use these names for my novel or game characters?",
    a: "Yes, absolutely. Random name generators are widely used in creative writing, tabletop RPGs, video game development, and scriptwriting. The names are common, culturally neutral American English names, suitable for contemporary fiction and most fantasy or historical settings that use English names.",
  },
  {
    q: "Are the names culturally diverse?",
    a: "The current name pool focuses on common American English first names and common English/Spanish/Asian surnames found in US census data. It represents the most common names in the United States but is not exhaustive of all cultures. The last name list includes names of Hispanic, Vietnamese, Korean, and South Asian origin, reflecting real US demographic diversity.",
  },
  {
    q: "Can I generate just first names or just last names?",
    a: "Currently, the generator always produces full names (first + last). When you copy individual names, you can manually use just the first or last name portion. A future update may add separate first-name-only and last-name-only modes.",
  },
  {
    q: "How random are the names?",
    a: "Each name is selected using JavaScript's Math.random(), which provides pseudo-random selection from the full name pools. With over 80 male first names, 100 female first names, and 100 last names, there are more than 18,000 possible unique name combinations. Generating 10 names at a time rarely produces a repeat.",
  },
  {
    q: "Is there a way to export the names to a spreadsheet?",
    a: "Click 'Copy All' to copy the full list as a newline-separated text. Paste it into any spreadsheet application (Excel, Google Sheets, Numbers) and use 'Text to Columns' (with space as the delimiter) to split first and last names into separate columns.",
  },
];
...
const RELATED_TOOLS = [
  ...
  { title: "Word Counter", slug: "online-word-counter", icon: <User className="w-5 h-5" />, color: 200, benefit: "Count words and characters" },
];
      ...
      "@graph": [
        {
          ...
          url: RANDOM_NAME_CANONICAL,
          description:
            "Free random name generator. Generate realistic American male, female, or mixed names for fiction, testing, design, gaming, or any creative project.",
        ...
        {
          "@type": "FAQPage",
          mainEntity: RANDOM_NAME_FAQS.map((item) => ({
            "@type": "Question",
            name: item.q,
        ...
        <section id="overview" className="rounded-2xl overflow-hidden border border-purple-500/15 bg-gradient-to-br from-purple-500/5 via-card to-violet-500/5 px-8 md:px-12 py-10 md:py-14 mb-10">
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            ...
            <span className="inline-flex items-center gap-1.5 bg-purple-500/10 text-purple-600 dark:text-purple-400 font-bold text-xs px-3 py-1.5 rounded-full border border-purple-500/20">
              <Zap className="w-3.5 h-3.5" /> Instant Generation
            </span>
          ...
          <div className="lg:col-span-3 space-y-10">
                  ...
                  <div
                    ...
                    className={
                      r.color === "purple"
                        ...
                        : r.color === "violet"
                          ? "p-4 rounded-xl bg-violet-500/5 border border-violet-500/20"
                          : r.color === "fuchsia"
                            ? "p-4 rounded-xl bg-fuchsia-500/5 border border-fuchsia-500/20"
                            : "p-4 rounded-xl bg-pink-500/5 border border-pink-500/20"
                    }
                  >
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span
                        className={
                          r.color === "purple"
                            ? "text-sm font-black text-purple-600 dark:text-purple-400"
                            : r.color === "violet"
                              ? "text-sm font-black text-violet-600 dark:text-violet-400"
                              : r.color === "fuchsia"
                                ? "text-sm font-black text-fuchsia-600 dark:text-fuchsia-400"
                                : "text-sm font-black text-pink-600 dark:text-pink-400"
                        }
                      >
                        {r.type}
                      </span>
            ...
            <section className="rounded-2xl bg-gradient-to-br from-purple-500/10 via-card to-violet-500/10 border border-purple-500/20 p-8 md:p-10 text-center">
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                <Link href="/text/lorem-ipsum-generator"
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div
                      ...
                      className={
                        r.color === "blue"
                          ? "flex items-center justify-between p-2.5 rounded-lg bg-blue-500/5 border border-blue-500/20"
                          : r.color === "pink"
                            ? "flex items-center justify-between p-2.5 rounded-lg bg-pink-500/5 border border-pink-500/20"
                            : r.color === "purple"
                              ? "flex items-center justify-between p-2.5 rounded-lg bg-purple-500/5 border border-purple-500/20"
                              : "flex items-center justify-between p-2.5 rounded-lg bg-violet-500/5 border border-violet-500/20"
                      }
                    >
                      ...
                      <span
                        className={
                          r.color === "blue"
                            ...
                            : r.color === "pink"
                              ? "text-xs font-black text-pink-600 dark:text-pink-400"
                              : r.color === "purple"
                                ? "text-xs font-black text-purple-600 dark:text-purple-400"
                                : "text-xs font-black text-violet-600 dark:text-violet-400"
                        }
                      >
                        {r.count}
                      </span>
